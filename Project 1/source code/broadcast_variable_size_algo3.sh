#!/bin/bash
#SBATCH --job-name=bcast_var_size3
#SBATCH --nodes=2
...
do
	...
	do
		...
		result_broadcast3=$(mpirun --map-by core -np $number_processes --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_bcast_algorithm 3 $OSU_BCAST -m $size -x $N_replica -i $N_replica | tail -n 1 | awk '{print $2}')
